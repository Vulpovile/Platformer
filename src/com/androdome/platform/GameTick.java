	...
	public void run()
	{
		while(running)
		{
			try {
				...
				if(!frame.gamepanel.collides(frame.player.location.x+5, 16+frame.player.location.y-1))
				{
					frame.player.velocity.y += 1;
				}
				
				if(!frame.gamepanel.collides(frame.player.location.x+5, 16+frame.player.location.y+frame.player.velocity.y-1))
				{
